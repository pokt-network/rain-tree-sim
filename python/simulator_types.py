...
class RainTreeConfigs:
    ...
    def __init__(
        self, num_nodes: int, t1_per: float, t2_per: float, shrinkage_per: float
    ):
        self.num_nodes = num_nodes
        self.t1_per = t1_per
        self.t2_per = t2_per
        self.shrinkage_per = shrinkage_per
        self.max_theoretical_depth = RainTreeConfigs._get_max_theoretical_depth(
            num_nodes
        )
    ...
    def _generate_addr_book(num_nodes: int) -> List[str]:
        return sorted(
            [f"node_{i+1}" for i in range(num_nodes)],
...
class RainTreeQueueElement:
    ...
    addr_book: List[
        str
    ]  # The addr book of the current node (i.e. list of sorted addresses)
